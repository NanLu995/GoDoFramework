...
当前不包含运行时改键、输入提示格式化和震动。`ActiveDevice` 在首次有效输入前为 `Unknown`；
这些剩余能力在独立批次验证后再开放，不虚报 GUIDE 已有但适配器尚未暴露的能力。
...
模拟事件过滤、虚拟摇杆归类和关闭清理，并验证 3 Actions × 1,000 次缓存采样为零托管分配。
真实手柄、窗口失焦、编辑器 Profile 制作流程和渲染/物理帧延迟仍需手动验证。
