...
SettingsService 管理音量、语言和平台显示偏好，与游戏进度存档分离。它自动检测当前平台并选择能力适配器，通过显式依赖使用 AudioService 和 SaveService。
...
- CommonOnly：未知平台安全降级，只启用通用能力并输出 Warning。
...
Windows 首版稳定基线已经完成。SettingsService、内部 Codec、Save/Audio 显式接入和 GoDoRuntime 注册均已落地；服务使用固定槽位 `godo-settings` 和数据版本 1，模块内部不通过 Services 横向查找依赖。

## 失败语义

- 非法音量、空 Locale、未知窗口模式和非正分辨率抛出参数异常，且不会更新 `Current`。
