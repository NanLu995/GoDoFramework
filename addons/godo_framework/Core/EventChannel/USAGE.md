...
`Once` 会在调用回调前标记移除，因此同类型事件重入或回调抛出异常都不会再次执行；它不会自动跟随任意 Node，事件可能永远不发生时，应交给 `EventScope` 管理，或在所有者退出时手动 `Off`。
...
- 派发期间新增的监听会延迟到最外层派发结束后生效；`Off` 会立即跳过本轮尚未执行的目标回调，底层列表变更仍延迟提交。
