...
- 复用同一 Remapper Binding，按 Context、Action 和设备提供稳定顺序的文本提示，并声明 `PromptQuery` 能力。
- 使用 SaveService 保存 GUIDE 配置、恢复备份并声明 `RebindingPersistence` 能力。
- 由 GoDoRuntime 统一更新和关闭后端。

当前不包含输入提示图标、手柄品牌图形和震动。`ActiveDevice` 在首次有效输入前为 `Unknown`。
...
GUIDE 适配器按 `GuideInputProfile.Bindings` 顺序筛选当前绑定；改键后的输入设备优先，解绑槽位使用默认输入判断所属设备并返回空文本。提示只在查询时格式化，不进入每帧采样。当前文本适合作为设置界面或无图标 UI 的回退，图标资源和本地化由游戏管理。

`Apply`、`RestoreDefault` 和 `LoadAndApply` 成功后发布 `InputBindingsChangedEvent`；失败并回滚时不发布。UI 同时监听该事件和 `InputDeviceChangedEvent` 即可刷新提示。
...
- 提示查询只扫描指定 Context 的 Binding；无结果时复用空数组，有结果时创建返回数组，不影响 GUIDE 或 GoDo 的每帧采样路径。
