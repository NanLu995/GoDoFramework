...
宿主只在插件进入树时扫描一次 `res://addons/` 与 `res://addons/godo_framework/Integrations/` 的一级子目录，并读取固定名称 `godo_editor_extension.cfg`；不递归扫描、不轮询，也不使用 `_Process()`。清单必须提供唯一 `id`、显示名、精确匹配的宿主 API 版本和位于同一适配包目录内的 GDScript。清单按 `menu_order`、扩展 ID 排序，单个扩展失败只记录到“编辑器扩展状态...”，不阻断核心菜单或其他扩展。
...
打开“项目 → 工具 → GoDo Framework → 资源 (Resources) → 校验资源清单 (Validate Resource Manifest)...”，选择 `.tres` 或 `.res` 格式的 `ResourceManifest` 资源。校验器只读取资源并输出报告，不生成清单、不修复路径，也不写入任何项目文件。
