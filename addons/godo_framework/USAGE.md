...
本插件是 GoDo 的项目安装助手、健康检查工具与资源清单校验入口，只负责检查、安装和卸载 `GoDoRuntime` Autoload，以及只读校验 `ResourceManifest`。它不创建业务场景或 UI，不修改 C# 项目文件，也不参与导出后的游戏运行。
...
2. 打开“项目 → 工具 → GoDo Framework → 设置 (Setup)...”窗口。
