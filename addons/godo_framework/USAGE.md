...
本插件是 GoDo 的项目安装助手、健康检查工具、资源清单校验入口与编辑器扩展宿主。它负责检查、安装和卸载 `GoDoRuntime` Autoload，只读校验 `ResourceManifest`，并通过 `godo_editor_extension.cfg` 注册可选集成与框架 Tools 的顶部 `GoDo` 菜单项。DataTable 离线编译前端及编辑器入口详见 `Tools/DataTable/USAGE.md`。这些工具不创建业务场景或 UI，不修改 C# 项目文件，也不参与导出后的游戏运行。
...
宿主只在插件进入树时扫描一次 `res://addons/`、`res://addons/godo_framework/Integrations/` 与 `res://addons/godo_framework/Tools/` 的一级子目录，并读取固定名称 `godo_editor_extension.cfg`；不递归扫描、不轮询，也不使用 `_Process()`。清单必须提供唯一 `id`、显示名、精确匹配的宿主 API 版本和位于同一包目录内的 GDScript。清单按 `menu_order`、扩展 ID 排序，单个扩展失败只记录到“编辑器扩展状态...”，不阻断核心菜单或其他扩展。
...
编辑器顶部工具栏提供原生样式的 `GoDo` 下拉入口，悬停可查看完整名称 `GoDo Framework`；其中的“创建资源清单 (Create Resource Manifest)...”可以创建一个空的 `.tres` / `.res` 格式 `ResourceManifest`。创建失败通常代表目标项目尚未完成 C# 编译，或已启用的插件尚未重新加载，导致清单脚本无法被编辑器加载或实例化。
...
打开编辑器顶部“GoDo → 校验资源清单 (Validate Resource Manifest)...”，选择 `.tres` 或 `.res` 格式的 `ResourceManifest` 资源。校验器只读取资源并输出报告，不生成清单、不修复路径，也不写入任何项目文件。
...
- `EditorExtensionUiRegression.gd` 会在 Headless Editor 中真实触发已安装扩展的菜单，验证 GUIDE Input 与 Phantom Camera 报告非空，并确认健康状态下修改按钮禁用。
