...
具体输入插件通过核心外的 `IInputBackend` 适配包接入。例如 `addons/godo_guide_input/` 依赖 GoDo Input、Save 与 G.U.I.D.E-CSharp，但 GoDo 核心不反向依赖该包。业务只使用 GoDo 语义 ID、InputFrame、可选 `IInputRebinding` 与 `IInputRebindingPersistence`，不持有插件类型。
