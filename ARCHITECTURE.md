...
具体输入插件通过核心外的 `IInputBackend` 适配包接入。例如 `addons/godo_guide_input/` 依赖 GoDo Input 与 G.U.I.D.E-CSharp，但 GoDo 核心不反向依赖该包。业务只使用 GoDo 语义 ID、InputFrame 与可选 `IInputRebinding`，不持有插件类型。
...
| Service | Input | 语义 Action 当前帧快照、Context 栈、可选运行时重绑定与可替换后端边界 | Core、Godot 值类型、`IInputBackend` | `IInputService` | 核心接入 |
