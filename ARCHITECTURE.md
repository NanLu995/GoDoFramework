...
2. **唯一例外是 ErrorHandler**。任何模块（包括 Core 层之间、业务层）都可以直接调用 `GoDo.ErrorHandler.Report(...)` 报错，不需要绕事件系统。ErrorHandler 只接收调用，不会反过来依赖任何其他模块——它的 `OnError` 用原始 C# event 委托实现，不经过 EventChannel，避免循环依赖。
...
**架构约束（重要）**
- ErrorHandler **不依赖 EventChannel**，也不依赖任何其他 Core 模块。它的 `OnError` 用原始 C# `event` 委托实现。这是为了避免"EventChannel 出错要用 ErrorHandler 报错，ErrorHandler 又用 EventChannel 分发"这种循环依赖。
...
**职责**
- 日志分级，Release 可关闭
- 与 ErrorHandler 联动（ErrorHandler 内部用 Log 输出格式化后的内容，但 Log 本身不反向依赖 ErrorHandler）

**架构说明**：Log 和 ErrorHandler 是近亲，建议一起实现。Log 是更通用的"输出"能力，ErrorHandler 是更专门的"异常处理"能力，ErrorHandler 内部会调用 Log 来实际打印，但 Log 不需要知道 ErrorHandler 的存在。
...
**待补充设计决策**：是否要支持非 Node 的纯 C# 对象池（不是所有需要池化的东西都是场景节点）？节点归还时的"自动重置状态"具体重置哪些字段，是否需要框架使用者自定义重置逻辑（比如实现一个 `IPoolable` 接口）？
...
**目录调整说明**：Log 和 Tick 从原来的 Tools 挪进了 Core，因为它们是框架地基级别的能力，跟 ErrorHandler、EventChannel 是同一性质。Tools 现在只保留相对独立、随时可插空做的内容。
...
- Core 层模块之间禁止互相 Get，通信走 EventChannel（ErrorHandler 除外，见上方架构约束）
- ServiceLocator 只用于业务层访问框架服务，不用于框架内部模块互访
