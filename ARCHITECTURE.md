...
业务层（具体游戏）
        ↓
Runtime Services（Scene / Audio / UI / Save / Settings / Procedure）── 对应第 4 节表格中的 Service 层
        ↓
...
> 上图只描述主依赖链路。Tools（Config）、Editor（Installer）、Development（Diagnostics）三层不在这条链路上，属于侧挂关系：它们可以依赖 Core / Foundation，但不允许被 Runtime Services 或业务层之外的运行时模块反向依赖（见规则 6）。第 4 节表格中的"层"一列与本图术语对应关系已在表头注明。
...
当前长期服务注册顺序为 Scene、Audio、UI、Save、Settings、Procedure；Settings 通过构造函数显式依赖 Audio 与 Save。退出时反向注销。Services 只保存引用，不负责创建、释放或推断生命周期。Procedure 只注册顶层流程切换服务，GoDoRuntime 不主动进入任何业务流程。
...
>
> **状态图例**：`稳定基线` = 已长期使用、接口基本不再变动；`首版完成` = 首个可用版本已完成，接口可能仍会调整；`Windows 稳定基线` = 目前仅在 Windows 平台验证为稳定，其他平台的行为尚未确认，跨平台前需在对应 `USAGE.md` 中核实；`已采用` = 专用于 GoDoRuntime 这类框架入口本身——它是固定的运行时骨架而非可替换/可独立演进的模块，因此不套用其余三种成熟度分级。
...
| Editor | Installer | GoDoRuntime Autoload 的显式安装与健康检查 | Godot Editor API | `项目 → 工具` | 稳定基线 |
...
- Debug 专属实现使用 `#if DEBUG`，覆盖编辑器 Debug 与 ExportDebug，不进入 Release / ExportRelease。
  **⚠️ 待核实**：`FRAMEWORK_OVERVIEW.md` 中 EventChannel 的描述使用的是 `#if GODOT_DEBUG`，与此处 `#if DEBUG` 不一致。两个符号触发条件不完全相同，需要对照实际源码确认框架统一使用哪一个，并同步修正另一份文档，避免不同模块用了不同的条件编译符号。
