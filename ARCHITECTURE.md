...
业务层（具体游戏）
        ↓
Runtime Services（Scene / Audio / UI / Save / Settings）
        ↓
...
当前长期服务注册顺序为 Scene、Audio、UI、Save、Settings；Settings 通过构造函数显式依赖 Audio 与 Save。退出时反向注销。Services 只保存引用，不负责创建、释放或推断生命周期。
...
| Service | UI | 屏幕空间 UI 的 Scene、View、Modal 层与返回栈 | ResourceHub、Core | `IUiService` | 首版 |
...
| Editor | Installer | GoDoRuntime Autoload 的显式安装与健康检查 | Godot Editor API | `项目 → 工具` | 首版 |
