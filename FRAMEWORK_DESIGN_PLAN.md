...
接下来的能力建设分为两条轨道：一条是按真实需求单独设计 Tick、Input、本地化、资源扩展等运行时能力；另一条是逐步补齐 API 兼容与迁移、跨平台验证、资源与编辑器审计、自动化验证和发布流程。Procedure 已按“顶层游戏流程状态机”完成首版，但不先抽象通用 StateMachine；UI 已完成首轮实际项目验证并进入稳定基线。
