...
| UI | 首版 | 管理屏幕空间 Control 的 Scene、View、Modal 层与返回栈；不承担主场景和游戏流程 |
| Diagnostics | 稳定基线 | 默认显示紧凑 FPS 按钮，可触摸展开只读摘要；Release 不创建节点 |
| EditorPlugin | 首版 | 显式安装与检查 GoDoRuntime Autoload；不自动修改项目，不进入运行时依赖 |
...
3. 通过 GitHub Release 发布 ZIP，压缩包只包含完整的 `addons/godo_framework/`；
