...
| UI | 延后 | 先由真实游戏验证层级、模态和返回栈需求 |
| Diagnostics | 候选优先 | 评估轻量只读 Debugger 与模块诊断扩展点 |
...
| 4. 开发体验 | 轻量 Debugger、经真实需求确认的 UI | 尚未开工 |
...
暂停新增模块，先按 Core、Resources、Pool、Scene、Audio、Save、Settings、Config 的顺序学习和复盘现有基线。完成复盘后优先讨论轻量 Debugger 的观测需求；Procedure 和 UI 等待真实项目证据，不直接按早期设想开工。
