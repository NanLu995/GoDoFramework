...
> **状态图例**：`稳定基线` = 接口和行为已长期使用、基本不再变动；`首版完成` = 首个可用版本已完成，接口仍可能调整；`Windows 稳定基线` = 仅在 Windows 上验证为稳定，其他平台待验证；`已采用` = 用于 GoDoRuntime 这类框架入口本身，它不是一个可独立替换/演进的"模块"，而是固定的运行时骨架，因此不套用其余三种成熟度分级。
>
| 模块 | 状态 | 首版边界或下一步 |
|---|---|---|
...
| Debugger | 稳定基线 | 默认显示紧凑 FPS 按钮，可触摸展开只读摘要；Release 不创建节点 |
...
> **状态更新**：Debugger 已按下方设计意图完成首版并升级为第 4 节所列的"稳定基线"，不再属于"暂缓"状态。以下设计初衷作为该模块的历史决策记录保留，不再是待评估方向。
...
| 5. 候选评估 | Procedure、Tick、Input、本地化与远程资源 | Procedure 首版完成；其余未排期 |
...
继续避免新增泛化模块，用小型真实游戏验证现有基线并记录重复出现的缺口。Procedure 已按“顶层游戏流程状态机”完成首版，但不先抽象通用 StateMachine；Tick、Input 和本地化仍需明确需求后再单独设计。UI 已完成首轮实际项目验证并进入稳定基线。
