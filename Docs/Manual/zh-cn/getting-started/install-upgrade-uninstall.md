...
成功时提示框显示“GoDoRuntime 安装成功。”，窗口状态变为“已正确安装”。如果看到“安装调用已完成，但复查未通过”，不要手工再添加一份 Autoload；保留当前检查结果并查看 Godot 编辑器输出，先处理其中的项目配置或写入错误，再点击“重新检查”。
...
成功移除时提示框显示“GoDoRuntime Autoload 已卸载，框架文件未删除。”。如果看到“卸载调用已完成，但复查未通过”，先保留框架目录，查看检查结果和编辑器输出；只有确认精确匹配的 Autoload 已消失后，才继续禁用插件和删除目录。
