...
NotFound 是正常结果，不抛异常。`Exists` 只检查正式档或备份是否存在，不代表文件一定可读；可读性以 `Load` 为准。删除槽位会尝试删除正式、备份和临时文件；没有文件时返回 `false`，执行前必须让玩家确认。若部分文件删除失败，`Delete` 会抛 `SaveException`，但不会恢复已经删除的文件；此时重新检查槽位并决定是否重试或提示玩家。
