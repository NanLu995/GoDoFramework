...
也可以先打开 `GoDo Framework → 打开 GoDo Framework... → 编辑器扩展 → Friflo ECS`，在右侧直接查看依赖报告、提示和操作。重新检查、打开官网和安装结果都在提示栏反馈。当根目录只有一个普通 `.csproj`、依赖缺失且没有 `Directory.Packages.props` 时，“添加依赖...”按钮可用。确认窗口会显示实际项目文件和上述精确 `PackageReference`；确认后先在同目录创建不覆盖旧文件的 `.godo-backup` 备份，再写入并重新检查。已有备份时文件名会递增编号。
