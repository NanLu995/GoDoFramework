...
启用 GoDo Framework 编辑器插件后，从顶部菜单打开统一窗口，再进入“资源 → 资源清单”。页面会直接列出项目内现有清单；没有清单时可直接创建。选中清单进入管理窗口后，可以添加项目资源、编辑 ID、将路径转换为 UID、删除映射或执行只读校验；刷新、创建和切换清单都在主页完成。管理表格使用 `Id / Locator / UID Status`，写入后会重新读取磁盘内容，避免把尚未持久化的条目显示为成功。

添加资源时，工具默认用去掉 `res://` 和扩展名的路径作为 ID。提交前将重要条目改为稳定业务语义，例如 `ui/icon_close`。删除映射不会删除实际资源文件。

添加资源和生成缺失 UID 都需要确认后才会写入；校验操作不会修复或修改文件。
