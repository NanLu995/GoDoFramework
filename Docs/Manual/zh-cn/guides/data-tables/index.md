...
DataTable 是开发期编译工具：它读取 UTF-8 CSV 和 JSON Profile，检查类型、主键、范围与跨表外键，然后生成二进制 `.gdtb` 和强类型 C# 读取代码。游戏运行时读取生成物，不解析原始 CSV。
...
只手工维护 Build Config、Profile 和 Sources。`Generated` 是工具输出，不应手工编辑；是否提交生成物由团队决定，但 CI 必须能验证它们没有过期。

输入 CSV 使用 UTF-8，可带 BOM。列名必须与 Profile 字段名一致。
...
结构发生不兼容变化时递增表的 `schema_version`。跨端数据协议发生约定变化时由项目递增 `protocol_version`。这些版本不会自动迁移旧二进制或网络连接。
...
所有路径都相对于这个 JSON，使用正斜杠，不能是绝对路径或包含 `..`。C# 文件必须位于数据输出目录之外，因为数据输出目录生成时会整体替换。
...
成功返回退出码 0，数据、Profile、路径或 I/O 错误返回 1。工具会在全部校验通过后再提交文件；失败不会覆盖上一次成功生成的产物。内容没有变化的 C# 文件不会重写，避免无意义的 Godot/.NET 重编译。

生成物包括：

- 每张表的 `<TableId>.gdtb`。
- 完整及 Client/Server Manifest 与 Debug JSON。
- 规范化 IR 和构建报告。
...
窗口默认寻找 `res://DataTables/datatable.build.json`。可以执行“检查全部”“生成全部...”或“生成选中表...”。生成操作会先展示目标并要求确认，完成后通知 Godot 扫描新文件。

Python 路径只保存在本机 EditorSettings，不写入项目配置。团队和 CI 仍应使用 Build Config 保持输入与输出路径一致。

## 7. 在游戏中读取生成表

生成代码位于 Profile 指定的命名空间。当前实验生成器提供强类型 Loader 和每表查询类型，例如：
...
普通 preset 选择 Client；带 `dedicated_server` feature tag 的 preset 选择 Server。Release 包只映射目标 `.gdtb` 和 `manifest.json`，Debug 额外包含目标 `debug.json`，CSV、Profile 和完整离线输出不应进入包。
...
DataTable 当前主要通过生成代码使用，尚未提供稳定的 GoDo public runtime API；升级前应阅读该版本的生成报告并重新运行完整验证。
