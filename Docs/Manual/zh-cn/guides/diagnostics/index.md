...
Audio 页面通过结构化状态区分 BGM 加载、播放、暂停、Crossfade、自然结束和停止，并给出当前已提交资源键。SFX 卡片分别显示非空间与 3D 池的活跃、等待、已准备声部、容量，以及服务初始化以来累计的拒绝和优先级抢占次数；3D 详情还显示当前目标跟随数量与独立上限。下方显示 Master、BGM、SFX 三组线性音量。任一已准备数量低于副本实测突发预算时，应在加载流程预热对应池而不是战斗帧扩容；跟随数长期顶住上限时，应先区分短音效与持续移动声，再按目标设备实测调整预算；拒绝持续增长通常表示同资源限制或全局/跟随预算正在生效，抢占增长表示高优先级请求替换了同一池中的较低优先级声音。
...
Procedure 页面显示当前流程、进入或退出阶段、待处理请求及被首请求仲裁拒绝的目标，详情保留上一个流程、最近成功和最近一次失败。独立顶层“运行链路”把 Procedure、Scene 与 UI 的关键状态放在同一视图，适合排查“流程已切换但场景或界面没有到位”。它是 Debugger 自身的只读联合视图，不是 Runtime Service。失败摘要包含结构化阶段，只保存有限文本且不持有异常对象；这些诊断状态不会进入 Release。
