...
Procedure 页面显示当前流程、进入或退出阶段、待处理请求及被首请求仲裁拒绝的目标，详情保留上一个流程、最近成功和最近一次失败。Flow 页面把 Procedure、Scene 与 UI 的关键状态放在同一视图，适合排查“流程已切换但场景或界面没有到位”。失败摘要包含结构化阶段，只保存有限文本且不持有异常对象；这些诊断状态不会进入 Release。
