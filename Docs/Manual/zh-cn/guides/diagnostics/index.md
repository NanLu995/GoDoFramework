...
异常被包装时不需要丢掉上层语义：上层消息描述哪个操作失败，最内层异常描述直接原因。Godot 控制台会始终附带有界的 `Cause` 摘要，所以 Release 也能看到根因；Debug 构建还会输出完整 `ExceptionChain`。ErrorHub 只负责保真呈现，不会根据异常类型自动猜测修复方法，因此最内层异常消息应尽量写清缺少的资源、无效值或建议检查项。
...
- 折叠状态只显示 FPS；出现 Warning 或 Error 时文字会按最高严重度变色，具体数量在概览中查看。鼠标或单指拖动该入口可移动位置，移动超过 8 个逻辑像素时不会误触展开。
...
- 点击或轻触入口展开；入口靠近屏幕边缘时，完整面板会优先保持当前尺寸并自动移入视口，折叠后入口回到原位置。鼠标或单指拖动标题栏可移动窗口，拖动右下角“拖动调整大小 ↘”可调整整个 Debugger 尺寸；位置只在本次运行中保留，位置或尺寸不合适时点击“重置”。
- 展开时每 0.25 秒刷新当前页面；折叠时不会创建模块快照。
- Scheduler 页面除汇总指标外，还列出最多 64 条活动任务及最近 16 条结束记录，可用 Owner、存活时间和结束原因定位跨场景残留；这些记录只存在于 Debug 构建。
- 面板只读，不允许修改服务或游戏数据。
...
Events 页面顶部汇总事件类型和监听器数量。搜索框可按事件短名称或完整类型名过滤；选择一项后会显示完整类型名，并列出最多 64 个当前监听来源的回调方法、`On` / `Once` / `Bind` / `EventScope` 注册方式、Owner、优先级和已注册时长。回调与 Owner 的完整名称或 Node 路径可在 Tooltip 中查看。`Off`、`Once` 派发、Node 退出树或 `EventScope.Dispose()` 后，对应来源会立即消失。该页面不记录事件触发或解绑历史，也不会捕获调用栈。

Input 页面用状态卡显示当前后端、活动设备、采样序号和 Action 数量，并显示 Context 与路由修订号。Context 表区分 Base、旧 LIFO 和 Lease 所有权、Token 与当前有效性；Router 表按优先级列出 Scope 和 Binding 数量。Action 表包含值、Idle/Ongoing/Performed 状态、同窗迁移、持续时间、进度和重触发门禁；搜索过滤完整快照，最多显示前 32 个匹配项。页面只读，不记录原始逐键历史。

Audio 页面通过结构化状态区分 BGM 加载、播放、暂停、Crossfade、自然结束和停止，并给出当前已提交资源键。框架内置 AudioService 执行 Play、Crossfade 或 FadeOut 时，状态说明还显示从请求创建开始的存活时间；Stop、完成、取消或被新请求替代后立即移除。自定义 `IAudioService` 继续显示接口状态，但不要求提供这项内部诊断。SFX 卡片分别显示非空间与 3D 池的活跃、等待、已准备声部、容量，以及服务初始化以来累计的拒绝和优先级抢占次数；3D 详情还显示当前目标跟随数量与独立上限。下方显示 Master、BGM、SFX 三组线性音量。任一已准备数量低于副本实测突发预算时，应在加载流程预热对应池而不是战斗帧扩容；跟随数长期顶住上限时，应先区分短音效与持续移动声，再按目标设备实测调整预算；拒绝持续增长通常表示同资源限制或全局/跟随预算正在生效，抢占增长表示高优先级请求替换了同一池中的较低优先级声音。这项请求计时只存在于 Debug 构建。

Scene 页面用状态卡显示当前场景、节点数量、切换状态和进度，详情表显示正在加载和最近切换的资源键及结果。切换期间“当前耗时”从请求开始持续增长，成功、失败或取消后恢复为 `—`；“最近耗时”保留最近一次结果的总耗时。节点数只在停留于该页面时每秒重算一次；SceneService 未注册或自定义实现不支持 Debug 快照时，会显示明确的降级状态。这些计时字段只存在于 Debug 构建。

Resources 页面用统计卡显示活动加载、同步/异步请求、同键合并和成功/失败数量。活动请求表按资源 key 稳定排序，最多显示前 32 条，并显示从唯一底层加载操作首次请求开始的存活时间；同 Key、同类型的后续合并不会重置计时。最近请求在内存中保留 32 条，页面按最新优先显示 8 条。这里显示的是 ResourceHub 加载操作和有限历史，不代表 Godot 全局缓存、Resource 引用寿命或最终持有者，也不是永久资源日志。这些计时与历史只存在于 Debug 构建。

Pool 页面显示当前仍存活的 NodePool 类型、空闲数、活动数和空闲容量，以及所有已登记池的合计。活动租借表最多显示 64 个节点的实例身份、当前父节点、状态和租借时长，Tooltip 提供 PackedScene 与父节点路径；`Release()` 后对应节点立即移除。它只在 Debug 构建中以弱引用观察 Pool，不会替你创建、关闭或回收节点；`Dispose()` 后对应 Pool 会立刻从页面消失。活动数量持续不回落时，优先查看存活时间较长、已脱离或等待删除的条目，再检查归还路径和场景退出顺序。
...
DataTable 页面显示已发布数据集、缓存表、当前加载和累计失败数量。数据集树可以展开查看表 ID 与实际缓存类型，加载中数据集在原有详情列显示表级进度、运行时目录和从加载开始计算的存活时间；成功、失败或取消后存活文本立即消失。最近结果记录加载成功、取消、失败与卸载。页面最多显示 32 个数据集和 64 张表，最近结果保留 16 条并显示最新 8 条。状态版本未变化时，页面只原地更新已显示顶层加载行，不重新采集快照、不重建树，也不遍历表子项。这些诊断只存在于 Debug 构建，不会让 Release 持有额外起点、Tree 元数据或历史。

UI 页面显示 Scene 界面数量、View 和 Modal 的栈深度，以及当前最上层的 View 或 Modal。列表按栈顶优先显示每个受管理节点、打开时的资源 Key 和显示/隐藏状态；未完成异步打开请求逐条显示层、UiId 或 Direct、资源、阶段、`configure` 来源类型和存活时间。实例委托显示目标类型，静态委托标为静态，未提供 `configure` 时显示未知；Tooltip 提供完整来源。请求明细最多 64 条但状态保留真实总数，成功、失败或取消后立即消失，不记录调用栈和请求历史。UiService 未注册、自定义实现不支持快照或业务绕过服务直接释放节点时，页面会给出明确状态。这些诊断只存在于 Debug 构建。

Procedure 页面显示当前流程、进入或退出阶段、待处理请求及被首请求仲裁拒绝的目标，详情保留上一个流程、最近成功和最近一次失败。进入或退出期间“当前耗时”从本次切换开始持续增长，并发拒绝不会重置它；切换结束后恢复为 `—`，“最近耗时”保留本次总耗时。独立顶层“运行链路”把 Procedure、Scene 与 UI 的关键状态放在同一视图，适合排查“流程已切换但场景或界面没有到位”。它是 Debugger 自身的只读联合视图，不是 Runtime Service。失败摘要包含结构化阶段，只保存有限文本且不持有异常对象；这些诊断状态不会进入 Release。

控制台把 Debug、Info、Warning、Error 和 Fatal 按时间顺序显示为一条连续日志流，不添加“最近警告 / 错误”等分区标题。顶部使用带数量的 All、Debug、Info、Warning 和 Error 标签。默认显示 All；点击某个等级可立即只看该等级，继续点击其他等级可组合筛选，点击 All 恢复全部。Warning 日志整行显示为黄色，Error/Fatal 整行显示为红色。搜索会扫描完整内存历史；结果超过一页时，使用“上一页 / 下一页”翻页，或点击右侧独立的“最新日志”直接返回最后一页并滚到底部。位于最新页且未暂停时，新日志会自动跟随到底部；手动向上滚动会停止跟随并启用“最新日志”，手动滚到底部或点击该按钮后恢复跟随。暂停后停止自动刷新和滚动。“复制”仅复制当前筛选、搜索和分页下正在显示的纯文本。搜索框只在点击后占用输入焦点，提交搜索或离开控制台后会释放焦点。分页状态旁的日志文件名可以点击，Windows 会在资源管理器中定位并选中文件；文件按钮与翻页按钮保持相同高度，悬停文件名可查看完整路径、写入状态、已刷新大小、累计丢弃数和失败原因。

控制台页面只保留有限的最近记录：LogHub 使用 1000 条环形历史，ErrorHub 摘要容量为 16；错误摘要包含上下文和有界根因，两者都可搜索，但不持有原始异常或完整堆栈。筛选和搜索扫描两类历史，按时间合并后每页最多显示 100 条匹配记录。连续相同日志会聚合为带首次/最后时间的 `×次数` 条目；Godot 输出窗口每秒最多接收 100 条 LogHub 文本，被抑制的文本仍保留在 Debugger 历史中。它是快速观察工具，不是完整日志存档或性能分析器。

## 查看跨会话滚动日志

GoDoRuntime 会优先把日志写到 `user://logs/godo_framework.log`，并在每次进程启动时先按 UTC 归档时间保存上次运行的主文件。如果该文件已被另一个运行实例占用，新实例会自动改写到 `godo_framework.<进程号>.log`，Debugger 中的文件按钮也会指向这个实际文件。单个文件达到 2 MiB 后滚动，最多保留四个历史文件；归档文件使用 `godo_framework.20260910T143012.1234567Z.log` 形式，进程专属日志的历史文件会保留相同进程号。Debug 构建记录 Debug、Info、Warning、Error 和 Fatal；Release 只记录仍然存在的 Warning、Error 和 Fatal。带异常的记录会保存根因摘要和完整异常链，便于离线排障；文件可能包含本机路径等技术信息，上传前应执行隐私审查。
