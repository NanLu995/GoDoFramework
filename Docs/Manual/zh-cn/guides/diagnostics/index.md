...
控制台页面只保留有限的最近记录：LogHub 使用 1000 条环形历史，ErrorHub 摘要容量为 16；筛选和搜索扫描两类历史，按时间合并后每页最多显示 100 条匹配记录。连续相同日志会聚合为带首次/最后时间的 `×次数` 条目；Godot 输出窗口每秒最多接收 100 条 LogHub 文本，被抑制的文本仍保留在 Debugger 历史中。它是快速观察工具，不是完整日志存档或性能分析器。
