...
- Owner 自动取消及回调异常取消计数。
- 各时间域下一任务的剩余时间。
- 最多 64 条活动任务的自动标签、Owner、状态、存活时间和剩余时间。
- 最近 16 条结束记录，并区分正常完成、主动取消、Owner 退出、Token 取消、框架关闭和回调失败。

任务标签由调度类型和 callback 方法自动生成；Owner 路径只作为有界文本显示，不会因为诊断历史继续持有已经退出的 Node。存活时间使用单调时钟，仅供排查，不改变所选 ScheduleClock 的到期语义。
