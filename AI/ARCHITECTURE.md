...
| Editor | Installer / Validator / Extension Host | GoDoRuntime Autoload 的显式安装、健康检查、ResourceManifest 只读校验，以及 Integrations / Tools 编辑器扩展的受控发现与菜单注册；DataTable 通过该宿主执行离线检查、全量生成与带基线保护的单表生成 | Godot Editor API、通用扩展清单 | 顶部 `GoDo` 菜单 | 首版完成 |
