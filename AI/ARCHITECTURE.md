...
> 本文只记录当前已经采用的架构事实、依赖规则和关键决策。模块 API、失败语义与验证结果见对应目录的 `USAGE.md`；未来路线见 `AI/FRAMEWORK_DESIGN_PLAN.md`。
...
当前长期服务注册顺序为 Scheduler、Scene、Camera、Input、Audio、Localization、UI、Save、Settings、Procedure；Localization 在 GoDoRuntime 的 `_EnterTree()` 中先应用默认 Locale，使业务 UI 初始化前已有确定语言。Settings 通过构造函数显式依赖 Audio、Save 与 Localization。Scheduler 是 `ProcessMode.Always` 的单一 Node，只依赖 Core 与 Godot 时间/节点生命周期，不通过 Services 查找其他服务。Camera 当前只依赖 Core 与 Godot Node 生命周期，不直接依赖 Scene 或具体摄像机插件。Input 当前只依赖 Core、Godot 值类型和 `IInputBackend` 边界；未安装后端时保持未就绪且不执行每帧采样。退出时先关闭并注销 Scheduler，使未完成等待可靠取消，再反向清理其余服务。Services 只保存引用，不负责创建、释放或推断生命周期。Procedure 只注册顶层流程切换服务，GoDoRuntime 不主动进入任何业务流程。

具体摄像机插件通过核心外的可选 `CameraRig` 适配包接入。例如 `addons/godo_phantom_camera/` 依赖 GoDo Camera 与 Phantom Camera，但 GoDo 运行时核心不反向依赖该包。其编辑器工具通过通用扩展清单接入唯一的 GoDo EditorPlugin，宿主只依赖扩展协议，不依赖具体适配包身份。

具体输入插件通过核心外的 `IInputBackend` 适配包接入。例如 `addons/godo_guide_input/` 依赖 GoDo Input、Save 与 G.U.I.D.E-CSharp，但 GoDo 运行时核心不反向依赖该包。业务只使用 GoDo 语义 ID、InputFrame、可选 `IInputRebinding` 与 `IInputRebindingPersistence`，不持有插件类型。适配包的编辑器检查器同样只通过通用扩展清单注册，不成为第二个 Godot EditorPlugin。
...
| Service | Input | 语义 Action 当前帧快照、Context 栈、可选运行时重绑定/持久化与可替换后端边界 | Core、Godot 值类型、`IInputBackend` | `IInputService` | 核心接入 |
...
| Editor | Installer / Validator / Extension Host | GoDoRuntime Autoload 的显式安装、健康检查、ResourceManifest 只读校验，以及可选适配包编辑器扩展的受控发现与菜单注册 | Godot Editor API、通用扩展清单 | 顶部 `GoDo` 菜单 | 首版完成 |
...
- 需要由业务层观察的稳定框架事实可直接作为 public `IEventMessage`，例如 Localization 的 `LocaleChangedEvent`；这类契约必须由所属模块维护并说明兼容性。
...
Settings 负责玩家 Locale 的选择、内存快照与持久化；Localization 负责 Locale 规范化与有效性、TranslationServer 查询、实际语言应用和变更事实通知。业务通过 Settings 切换语言，不能绕过持久化边界直接要求 Localization 修改当前 Locale。
