...
3. **DataTable**：阶段 A / B 实验原型已完成 CSV 校验、强类型代码生成、紧凑 v2 二进制、损坏文件拒绝、Godot Zstd 候选、确定性报告与 Windows Debug / Release IL/JIT 对比；阶段 C.1 至 C.6 已提供可分发 Python 编译前端、可移植 Build Config、整套 `generate` / 不写入的 `check`、安全单表生成、只读 `verify-generated`、Client / Server audience 隔离产物、ExportPlugin 过滤、可靠发布包装命令，以及面向非 Godot 服务端的语言无关 Manifest 契约与兼容比较，生成读取器已验证绝对路径、项目目录和 PCK 内 `res://`。`Auto` 当前只建议并保守选择未压缩，移动端与完整 ExportRelease 验证后再确定阈值。下一步用真实业务表验证单机存档 ID 和访问模式，再决定运行时 public API；加密明确后置。
