...
3. **DataTable**：按 `Docs/DataTableDesign.md` 使用一张真实 CSV 业务表验证格式校验、强类型代码生成、紧凑二进制、Debug 可读产物与 Release 排除源表，并分别验证单机存档 ID 和网络客户端 / 权威服务器摘要边界；压缩默认 Auto 且提供报告，加密明确后置。
