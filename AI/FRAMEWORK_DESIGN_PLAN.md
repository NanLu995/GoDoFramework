...
Input 的活动设备、有效 Context 和 Action 当前值已通过 Debug-only 快照接入 Debugger；提示查询作为后端可选能力显式暴露，不把图标或具体 UI 样式加入框架。Windows Demo3D 已通过真实手柄、设备切换、拔插后键盘切换、改键持久化、恢复默认与窗口失焦人工验收；其他平台和真实项目长期渲染/物理时序继续随开发验证。
