...
3. **DataTable**：阶段 A / B 实验原型已完成 CSV 校验、强类型代码生成、紧凑 v2 二进制、损坏文件拒绝、Godot Zstd 候选、确定性报告与 Windows Debug / Release IL/JIT 对比；阶段 C.1 / C.2 / C.3 已提供可分发 Python 编译前端、可移植 Build Config、整套 `generate` / 不写入的 `check`、基于完整基线校验和多文件回滚的安全单表生成，并通过唯一 GoDo EditorPlugin 后台执行检查与确认生成。`Auto` 当前只建议并保守选择未压缩，移动端与正式 ExportRelease 验证后再确定阈值。后续按 `Docs/DataTableDesign.md` 接入 CI 与 Export，并验证单机存档 ID 和网络客户端 / 权威服务器摘要边界；加密明确后置。
