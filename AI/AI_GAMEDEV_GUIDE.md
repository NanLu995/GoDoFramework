...
1. 修改 `addons/godo_framework/`：先读 `AI/ARCHITECTURE.md` 和对应模块 `USAGE.md`。
2. 参考 Demo3D：先读 `Templates/Demo3D/README.md`（如存在）和 `AI/PROJECT_STRUCTURE.md`，并确认已安装其可选依赖。
3. 新增流程、界面、存档或配置：先读 `AI/Recipes/` 下对应菜谱。
...
- `ResourceManifest` / `ResourceRegistry`：用项目自己定义的稳定语义 ID 映射资源定位；ID 不是 Godot 自动生成值，也不要求等于文件名。代码中的 ID 常量与清单条目必须保持一致，例如 `ui/main_menu -> res://Scenes/MainMenu.tscn`。
...
创建资源清单条目时，优先定义表达业务含义的 ID，而不是直接把路径当作业务 API。推荐使用小写和 `/` 分组，例如 `ui/main_menu`、`gameplay/hud`、`audio/click`。

- ID 由游戏项目开发者定义，并在同一组加载的 Manifest 中保持唯一。
- 代码调用方应通过集中定义的常量引用 ID，避免散落字符串和拼写错误。
- 资源移动通常只修改 Manifest 的 Locator；ID 保持不变。
- ID 改名属于兼容性修改，必须同步修改调用方和 Manifest，并完成回归验证。
- 自动生成的路径式 ID 可以作为初始值，但进入稳定业务代码前应调整为语义化名称。
